...
manhattan_band_cols <- c("#3F3F46", "#A1A1AA")
trait_cols <- c("#0072B2", "#D55E00", "#009E73", "#CC79A7", "#E69F00", "#56B4E9", "#000000")
...
make_overlaid_manhattan_with_legend <- function(df, p_cols, out_file) {
  ...
  abline(h = -log10(1e-5), col = "#111827", lty = "dotted", lwd = 1.2)
  abline(h = -log10(5e-8), col = "#BE123C", lty = "dashed", lwd = 1.2)
...
if (has_rmvp) {
  ...
  tryCatch({
    rMVP::MVP.Report(
      ...
      threshold.lty = c(1, 2),
      threshold.lwd = c(1, 1),
      threshold.col = c("#4B5563", "#B91C1C"),
      amplify = TRUE,
      bin.size = 1e6,
      chr.den.col = c("#56B4E9", "#E69F00", "#B91C1C"),
...
if (file.exists(rg_csv) && file.info(rg_csv)$size > 10) {
  ...
  if (nrow(rg) > 0) {
    ...
    par(mar = c(6, 6, 4, 4))
    image(1:n, 1:n, mat, col = colorRampPalette(c("#1b4965", "white", "#d93025"))(100), 
